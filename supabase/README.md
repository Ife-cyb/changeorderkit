...
2. Apply every file in `supabase/migrations` in filename order with the Supabase CLI or SQL editor. Run `20260722171653_pro_entitlements_foundation.sql` as one complete transaction—not statement by statement—so its backfill lock remains held until the quota triggers are installed. It must be applied before deploying the plan-usage UI and Free save limit.
...
The Free three-document limit is enforced by a private atomic per-user counter. The migration holds a write-blocking table lock while it backfills existing usage and installs the triggers. The `AFTER INSERT` trigger runs only when PostgreSQL actually creates a row, then increments the counter under row locking and raises `FREE_DOCUMENT_LIMIT_REACHED` when a non-Pro account would exceed three records. The failed statement rolls both the row and counter increment back. An `AFTER DELETE` trigger releases a slot; updates and conflict-driven upserts never consume one. Existing usage is backfilled without deleting or hiding records.
...
- `change_orders` has an `AFTER INSERT` limit trigger and an `AFTER DELETE` slot-release trigger.
