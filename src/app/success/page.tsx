import type { Metadata } from "next";
import Link from "next/link";
import { CheckCircle2, FileText } from "lucide-react";

export const metadata: Metadata = {
  title: "Paid pilot next step",
  description: "Return to ChangeOrderKit after submitting paid-pilot details.",
  robots: {
    index: false,
    follow: false
  }
};

export default function SuccessPage() {
  return (
    <section className="tool-shell py-14" aria-labelledby="success-title">
      <section className="utility-panel mx-auto max-w-2xl p-6 text-center">
        <CheckCircle2 className="mx-auto h-12 w-12 text-teal-700" aria-hidden="true" />
        <h1 id="success-title" className="mt-4 text-3xl font-black text-slate-950">
          Thanks for your interest.
        </h1>
        <p className="mt-3 text-base leading-7 text-slate-700">
          If you submitted a pilot application or payment through our external provider, we will
          follow up using the details you entered there. This return page does not verify payment
          or automatically unlock a feature.
        </p>
        <Link href="/" className="btn btn-primary mx-auto mt-6 w-fit">
          <FileText className="h-5 w-5" aria-hidden="true" />
          Use the free generator
        </Link>
      </section>
    </section>
  );
}
